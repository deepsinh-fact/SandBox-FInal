...
function NewgstChart() {
    const { GSTByLastDaysData, isGSTByLastDays, isGSTByLastDaysFetching } = useSelector(TBSelector);
    ...
    React.useEffect(() => {
        if (SelctChart == "yearly") {
            let monthCounts = {};
            GSTByLastDaysData.forEach(item => {
                const monthYear = moment(item.createdDate).format('YYYY-MM');
                if (!monthCounts[monthYear]) {
                    monthCounts[monthYear] = 0;
                }
                monthCounts[monthYear] += item.gstCount
            });
            ...
            while (currentWeekStart.isBefore(endOfMonth) || currentWeekStart.isSame(endOfMonth, 'week')) {
                ...
                if (currentWeekEnd.isAfter(startOfMonth) && currentWeekStart.isBefore(endOfMonth)) {
                    const weekDataCount = GSTByLastDaysData.filter(item => {
                        const itemDate = moment(item.createdDate);
                        return itemDate.isBetween(currentWeekStart, currentWeekEnd, null, '[]');
                    }).reduce((sum, item) => sum + item.gstCount, 0); // Sum the counts of items in this week
    ...
    }, [GSTByLastDaysData, SelctChart])

    React.useEffect(() => {
        dispatch(GSTByLastDays({ Days: "360" }))
    }, [])
    React.useEffect(() => {
        if (isGSTByLastDays) {
            dispatch(updateState({ isGSTByLastDays: false }))
        }
    }, [isGSTByLastDays])
    ...
    const chartOptions = {
        ...
        tooltip: {
            ...
            custom: function ({ series, seriesIndex, dataPointIndex, w }) {
                ...
                const calendarIconPath = "M19 4h-1V2h-2v2H8V2H6v2H5c-1.11 0-1.99.9-1.99 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V10h14v10zm0-12H5V6h14v2zm-7 5h5v5h-5v-5z";
                const iconColor = w.config.fill.gradient.colorStops[0][0].color;
                return `
                    <div class="bg-gst/40 border-gst border-2" style="padding: 10px; color: white; border-radius: 5px; font-size: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.2);">
                         <div style="margin-top: 5px; font-size: 12px; font-weight: bold;" class="bg-gst rounded text-white">
        ...
        <Card extra={"w-full h-full sm:overflow-auto px-6"}>
            <header className="relative flex items-center justify-between pt-4">
                <div className="text-xl font-bold text-navy-700 dark:text-white">
                    New GST By {moment(moment().format('YYYY-MM-DD')).subtract(12, 'months').format('YYYY')}-{moment().format('YYYY')}
                </div>
                <div className="mb-0 flex items-center justify-center">
                    <select
                        value={SelctChart}
                        onChange={(e) => setSelectdTab(e.target.value)} className="mb-3 outline-none mr-2 flex items-center justify-center text-sm font-bold text-gray-600 hover:cursor-pointer dark:!bg-navy-800 dark:text-white">
