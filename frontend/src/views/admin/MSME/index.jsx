...
const MsmeSearch = () => {
    ...
    let productsName = ServiceApiName.Msmesearch;
    ...
    const onSubmit = (data) => {
        ...
        if (ServiceRegExr.msmeRegExr.test(number)) {
            ...
            <div className='mt-0 grid h-full grid-cols-1 gap-5 md:grid-cols-2'>
                <Card extra={"w-full flex justify-end h-full pb-2 sm:overflow-auto px-6"}>
                    ...
                    <div className="mt-8 overflow-x-scroll xl:overflow-x-hidden">
                        <AllCountShowChart paring={isGetDashboardCategoryCountFetching} name={"MSME"} color={Infodata.color.msme} />
                    </div>
                </Card>

                <div>
                    <Card extra={"mb-5 z-5 overflow-hidden"}>
                        <SearchHistory ApiCallForRecentSearch={ApiCallForRecentSearch} products={recentSearches} name={"recent search history"} coloer={"msme"} />
                    </Card>
                    <Card extra={"mb-0 z-5 overflow-hidden"}>
                        <PopularsearchHistory ApiCallForPopularSearch={ApiCallForPopularSearch} products={PopularsearchData.MSMEDetail} name={"popular search history"} coloer={"msme"} />
                    </Card>
                </div>
            </div>
        </div>
