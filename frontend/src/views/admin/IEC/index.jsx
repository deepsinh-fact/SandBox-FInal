...
import { TBSelector, IESDetailApi } from 'Store/Reducers/TBSlice';


const IecSearch = () => {
    const { openNotification } = useNotificationContext();
    const { register, handleSubmit } = useForm()
    let productsName = ServiceApiName.Iecsearch;
    ...
    const onSubmit = (data) => {
        ...
        if (ServiceRegExr.iecRegExr.test(number)) {
    ...
    const ApiCallForRecentSearch = (data) => {
        ...
        const search = Service.toUpperCase(number);
        setrecentSearches(Service.GetRecentSearches(productsName));
        dispatch(IESDetailApi({ "searchNumber": search }));
    }
            ...
            <div className='mt-0 grid h-full grid-cols-1 gap-5 md:grid-cols-2'>
                ...
                <div>
                    <Card extra={"mb-0 z-5 overflow-hidden"}>
                        <PopularsearchHistory ApiCallForPopularSearch={ApiCallForPopularSearch} products={PopularsearchData.IecDetail} name={"popular search history"} coloer={"din"} />
                    </Card>
                </div>
